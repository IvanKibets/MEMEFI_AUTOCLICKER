...
<div class="row clearfix">
            ...
            <div class="body">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            @if($data->status==0)
                                ...
                                <div wire:ignore>
                                    <select class="form-control supplier" wire:model="id_supplier">
                ...
                <div class="table-responsive">
                    <table class="table c_list table-hover table-bordered">
                        ...
                        <tbody>
                            @if($data->status==0)
                                <tr style="background:#eeeeee45">
                                    ...
                                    <td class="text-center">
                                        @if($pajak_produk>0)
                                            <a href="javascript:void(0)" data-toggle="modal" data-target="#modal_add_pajak" class="text-center"><i class="fa fa-edit"></i></a>
                                        @else
                                            <a href="javascript:void(0)" data-toggle="modal" data-target="#modal_add_pajak" class="text-center"><i class="fa fa-plus"></i></a>
                                        @endif
                                        <!-- <input type="number" class="form-control text-right" min="0" wire:model="pajak_produk" /> -->
                                        {{format_idr($pajak_produk)}}
                                        @error('pajak_produk') <span class="text-danger">{{ $message }}</span> @enderror
                                    </td>
                                    <td>
                                        @php($price = ($pajak_produk and $price) ? ($price + $pajak_produk) : $price)
                                        {{(($price && $qty) ? format_idr(($price)*$qty) : '')}}
                                    </td>
                                    <td>
                                        ...
                                        <a href="javascript:void(0)" wire:loading.remove wire:target="addProduct" class="btn btn-info btn-sm" wire:click="addProduct"><i class="fa fa-plus"></i> Produk</a>
                                    </td>
                                </tr>
                            @endif
                            @php($total=0)
                            ...
                            @foreach($data->details as $k => $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($data->status==0)
                                            @livewire('purchase-order.editable',['field'=>'pajak','data'=>$item->pajak,'id'=>$item->id],key('pajak'.$item->id))
                                        @else
                                            {{$item->pajak}}
                                        @endif
                                    </td>
                                    <td class="text-right">Rp. {{format_idr(($item->price+$item->pajak)*$item->qty)}}</td>
                                    <td class="text-center">
                                        @if($data->status==0)
                                            <span wire:loading wire:target="deleteProduct({{$item->id}})">
                                                <i class="fa fa-spinner fa-pulse fa-2x fa-fw"></i>
                                                <span class="sr-only">{{ __('Loading...') }}</span>
                                            </span>
                                            <a href="javascript:void(0)" wire:loading.remove wire:target="deleteProduct({{$item->id}})" class="text-danger" wire:click="deleteProduct({{$item->id}})"><i class="fa fa-trash"></i></a>
                                        @endif
                                    </td>
                                </tr>
                        ...
                        @endif
                        <tfoot style="background: #eeeeee45;">
                            <tr>
                                <th colspan="8" class="text-right">Sub Total</th>
                                <td class="text-right">Rp. {{format_idr($sub_total)}}</th>
                                <th></th>
                            </tr>
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    @else
                                        Rp. {{format_idr($data->biaya_pengiriman)}}
                                    @endif
                                </th>
                                <th></th>
                            </tr>
                            <tr>
                                <th colspan="8" class="text-right">
                                    ...
                                    <label style="font-weight: normal;" class="ml-2">Manual <input type="radio" wire:model="type_pajak" value="2" /></label>
                                </td>
                                <td class="text-right">
                                    <span wire:loading wire:target="type_pajak">
                                        <i class="fa fa-spinner fa-pulse fa-2x fa-fw"></i>
                                        <span class="sr-only">{{ __('Loading...') }}</span>
                                    </span>
                                    @if($data->status==0)                                        
                                        ...
                                        @else
                                            <p>Rp. {{format_idr($pajak)}}</p>
                                        @endif
                                    @else
                                        Rp. {{format_idr($data->ppn)}}
                                    @endif
                                </th>
                                <th></th>
                            </tr>
                            <tr>
                                ...
                                <td class="text-right">
                                    Rp. {{format_idr($sub_total+$biaya_pengiriman+$pajak)}}
                                </th>
                                <th></th>
                            </tr>
                        </tfoot>
    ...
    <div wire:ignore.self class="modal fade" id="modal_add_pajak" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class="fa fa-plus"></i> Pajak</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true close-btn">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <div class="fancy-radio">
                            <label><input name="type_pajak" wire:model="type_pajak_item" value="1" type="radio" checked><span><i></i>Amount</span></label>
                        </div>
                        <div class="fancy-radio">
                            <label><input name="type_pajak" wire:model="type_pajak_item" value="2" type="radio"><span><i></i>Percentage</span></label>
                        </div>
                    </div>
                    <div class="form-group">
                        @if($type_pajak_item==2)
                            <input type="number" class="form-control" wire:model="type_pajak_persentase" placeholder="Persentase" />
                        @endif
                        <input type="number" class="form-control" wire:model="pajak_amount" placeholder="Amount" />
                    </div>
                </div>
                <div class="modal-footer">
                    <span wire:loading wire:target="cancel">
                    ...
                    <button wire:loading.remove wire:target="add_pajak" type="button" wire:click="add_pajak" class="btn btn-info"><i class="fa fa-check-circle"></i> Submit</button>
                </div>
            </div>
        </div>
    </div>
...
@push('after-scripts')
    ...
    <script>
        select_supplier = $('.supplier').select2();
        $('.supplier').on('change', function (e) {
            var data = $(this).select2("val");
            @this.set("id_supplier", data);
        });
        setTimeout(()=>{
            $('.supplier').val({{$id_supplier}}).trigger('change');
        },1000)
