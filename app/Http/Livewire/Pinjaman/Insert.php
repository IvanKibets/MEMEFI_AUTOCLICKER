...
class Insert extends Component
{
    ...
    public $jenis_pinjaman_id,$description,$jenis_pinjaman,$biaya_admin,$asuransi,$margin=0;
    public $items=[],$nominal_diterima;
    ...
    {
        if($this->jenis_pinjaman_id){
            $jenis_pinjaman = JenisPinjaman::find($this->jenis_pinjaman_id);
            $this->biaya_admin = $jenis_pinjaman->biaya_admin; 
            $this->asuransi = $jenis_pinjaman->asuransi; 
            $this->margin = $jenis_pinjaman->margin; 
        }
        
        if($this->pinjaman and $this->angsuran and $this->margin>0){
            $pembiayaan = $this->pinjaman;
            $this->items = [];
            $this->jasa = round(($this->margin*2)/24,2);
            ...
            for($item=0;$item<$this->angsuran;$item++){
                ...
                if($item==0) $this->angsuran_perbulan = $this->items[$item]['total'];
    ...
    public function save()
    {
        ...
        $data->nominal_diterima = $this->nominal_diterima;
        $data->proteksi_pinjaman = $this->asuransi;
        $data->save();
        ...
        foreach($this->items as $k => $val){
            ...
            $item->pembiayaan = $val['pembiayaan'];
            $item->save();
        }
