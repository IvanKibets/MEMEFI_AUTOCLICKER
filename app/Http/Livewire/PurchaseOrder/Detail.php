...
class Detail extends Component
{
    ...
    public $alamat_penagihan,$purchase_order_date,$delivery_order_number,$delivery_order_date,$disc,$pajak=0,$catatan;
    protected $listeners = ['reload'=>'$refresh'];
    public $nama_product, $list_product_supplier,$type_pajak=1,$pajak_produk;
    public $payment_date, $payment_amount, $file_bukti_pembayaran, $metode_pembayaran;
    public $data_invoice, $sisa_bayar_inv,$type_pajak_item=1,$pajak_amount,$type_pajak_persentase;
    public function render()
    {
        if($this->id_supplier){
            $this->list_product_supplier = SupplierProduct::where('id_supplier', $this->id_supplier)->orderBy('id','DESC')->get();
        }else{
    ...
    public function updated($propertyName)
    {
        ...
        if($propertyName=='id_supplier'){
            // find PO Supplier draft
            $find = PurchaseOrder::where(['id_supplier'=>$this->id_supplier,'status'=>0])->first();
            if($find and $this->data->id_supplier != $this->id_supplier){
                return redirect()->route('purchase-order.detail',$find->id);
            }
        ...
        if($this->product_id and $this->product_uom_id and $this->price=="" and $this->disc=="" and $this->pajak_produk==""){
            $find = PurchaseOrderDetail::where(['id_po'=>$this->data->id,'product_id'=>$this->product_id,'product_uom_id'=>$this->product_uom_id])->first();
            if($find){
                $this->product_uom_id = $find->product_uom_id;
                $this->price = $find->price;
                $this->disc = $find->disc;
                $this->pajak_produk = $find->pajak;
            }else{
                $this->price = '';
                $this->disc = '';
                $this->pajak_produk = '';
        ...
        if($propertyName=='type_pajak_persentase'){
            if($this->price>0){
                $this->pajak_amount = (($this->type_pajak_persentase / 100) * $this->price);
            }
        }
    }
    ...
    public function addProduct()
    {
        ...
        if(!$detail){
            ...
            $detail->pajak = $this->pajak_produk;
            $detail->save();
        }else{
            $detail->price = $this->price;
            $detail->disc = $this->disc;
        ...
        $this->price = 0;$this->disc = 0;$this->qty = 0;$this->pajak_produk=0;
    ...
    {
        $this->pajak_produk = $this->pajak_amount;
