...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        if($this->cetak_invoice)
            $data = $data->paginate(5000);
        else
            $data = $data->paginate(500);

        return view('livewire.transaksi.index')->with(['data'=>$data,'total'=>$total->sum('amount')]);
    }
    ...
    public function get_data()
    {
        ...
        if($this->cetak_invoice == true){
            $data->whereNull('invoice_transaksi_id')->whereNull('payment_date');
        }
