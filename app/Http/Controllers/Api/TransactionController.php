...
class TransactionController extends Controller
{
    ...
    public function store(Request $r)
    {
        ...
        $r->uang_tunai = replace_idr($r->uang_tunai) + $r->voucher_amount;
        ...
        if(isset($r->voucher_number)) {
            $data->voucher_number = $r->voucher_number;
            $data->voucher_amount = $r->voucher_amount;
            $voucher = Voucher::where('voucher_number',$r->voucher_number)->first();
            if($voucher){
                $voucher->status = 1;
    ...
    public function data(Request $r)
    {
        ...
        $transaksi = Transaksi::orderBy('id','DESC')->where('user_id',\Auth::user()->id);
    ...
    public function dataAnggota(Request $r)
    {
        $validator = \Validator::make($r->all(), [
            'token'=>'required'
        ]);
        ...
        $transaksi = TransaksiItem::select('transaksi_items.status','transaksi.id','transaksi.jenis_transaksi','transaksi.no_transaksi','transaksi.metode_pembayaran','transaksi.created_at','transaksi.amount',
                        'transaksi_items.harga','transaksi_items.qty','transaksi_items.description',\DB::raw('transaksi_items.id as transaction_item_id'),
                        'user_member.name','user_member.no_anggota_platinum'
                    )->orderBy('transaksi.id','DESC')
                    ->join('transaksi','transaksi.id','=','transaksi_items.transaksi_id');

        // if(isset($r->nama) || isset($r->no_anggota)){
        $transaksi->leftJoin('user_member','user_member.id','=','transaksi.user_member_id');
        // }
        
        $transaksi->where('transaksi.user_id',\Auth::user()->id);

        if(isset($r->nama)){
            $transaksi->where('user_member.name','LIKE',"%{$r->nama}%");
        }
        if(isset($r->no_anggota)){
            $transaksi->where('user_member.no_anggota_platinum',"LIKE","%{$r->no_anggota}%");
        }
        if(isset($r->jenis_transaksi)) $transaksi->where('transaksi.jenis_transaksi',$r->jenis_transaksi);
        ...
        foreach($transaksi->paginate(100) as $k => $item){
            ...
            $temp[$k]['no_transaksi'] = $item->no_transaksi;
            $temp[$k]['jenis_transaksi'] = $item->jenis_transaksi;
            $temp[$k]['no_anggota'] = $item->no_anggota_platinum;
            $temp[$k]['nama_anggota'] = $item->name;
            $temp[$k]['metode_pembayaran'] = $item->metode_pembayaran ? metode_pembayaran($item->metode_pembayaran) : '-';
            $temp[$k]['created_at'] = date('d-M-Y H:i',strtotime($item->created_at));
            $temp[$k]['status_pembayaran'] = $item->payment_date ? 'Lunas' : 'Belum Lunas';
            $temp[$k]['payment_date'] = $item->payment_date ? date('d-M-Y',strtotime($item->payment_date)) : '-';
            ...
            $temp[$k]['ppn'] = format_idr($item->amount * 0.11);
            $temp[$k]['total'] = format_idr($item->amount);
            $temp[$k]['harga'] = format_idr($item->harga);
            $temp[$k]['qty'] = format_idr($item->qty);
            $temp[$k]['produk'] = $item->description;
            $temp[$k]['transaction_item_id'] = $item->transaction_item_id;
            $temp[$k]['struk'] = route('transaksi.cetak-struk-kasir',$item->id)."#toolbar=0&navpanes=0&scrollbar=0";
        }
        
        return response()->json(['status'=>$this->status,'jenis_transaksi'=>$r->jenis_transaksi ,'data'=>$temp], 200);
    }

    public function voidItem(Request $r)
    {
        $validator = \Validator::make($r->all(), [
            'token'=>'required',
            'id'=>'required',
        ...
        try {
            $transaksi_item = TransaksiItem::find($r->id);
            if($transaksi_item){
                $transaksi = Transaksi::find($transaksi_item->transaksi_id);
                /**
                 * Jenis Transaksi Anggota
                 * Kuota Anggota kembali
                 * 
                 */
                if($transaksi->jenis_transaksi==1){
                    // Kembalikan saldo paylater anggota
                    $anggota = UserMember::find($transaksi->user_member_id);
                    $anggota->plafond_digunakan = $anggota->plafond_digunakan + $transaksi_item->total; 
                    $anggota->save();
                }

                # kembalikan stok
                $product = Product::find($transaksi_item->product_id);
                if($product){
                    $product->qty = $product->qty + $transaksi_item->qty;
                    $product->qty_moving = $product->qty_moving - $transaksi_item->qty;
                    $product->save();
                }

                $transaksi_item->qty = -$transaksi_item->qty;
                $transaksi_item->status = 2; // void
                $transaksi_item->save();
    ...
    public function submitAktualTunai(Request $r)
    {
        $validator = \Validator::make($r->all(), [
            'token'=>'required',
            'amount'=>'required'
        ...
        $dailyCash = DailyCash::whereDate('cash_date',date('Y-m-d'))->first();
        $dailyCash->cash_end = $r->amount;
        $dailyCash->save();

        return response()->json(['status'=>'success'], 200);
    }

    public function submitTunaiAwal(Request $r)
    {
        $validator = \Validator::make($r->all(), [
            'token'=>'required',
            'amount'=>'required'
        ...
        if(!$dailyCash) {
            ...
            $dailyCash->cash_date = date('Y-m-d');
        }
        $dailyCash->cash_start = $r->amount;
        $dailyCash->save();

        return response()->json(['status'=>'success'], 200);
    }

    public function dailyCash(Request $r)
    {
        $validator = \Validator::make($r->all(), [
            'token'=>'required',
        ]);
        ...
        $data = ['tunai_awal'=>0,'transaksi_tunai'=>0,'transaksi_kredit'=>0,'total_tunai'=>0,'aktual_tunai'=>0,'selisih'=>0];
        
        $dailyCash = DailyCash::whereDate('cash_date',date('Y-m-d'))->first();
        
        $transaksi_tunai = Transaksi::where(['status'=>1,'jenis_transaksi'=>2])->where('user_id',\Auth::user()->id)->sum('amount');
        $transaksi_kredit = Transaksi::where(['status'=>1,'jenis_transaksi'=>1])->where('user_id',\Auth::user()->id)->sum('amount');

        if($dailyCash){
            $data['tunai_awal'] = $dailyCash->cash_start;
            $data['transaksi_tunai'] = $transaksi_tunai;
            $data['transaksi_kredit'] = $transaksi_kredit;
            $data['total_tunai'] = $dailyCash->cash_start+$transaksi_tunai;
            $data['aktual_tunai'] = $dailyCash->cash_end;
            $data['selisih'] = $data['total_tunai'] - $dailyCash->cash_end;
        }

        return response()->json(['status'=>'success','data'=>$data], 200);
    }
    public function checkVoucher(Request $r)
    {
        $validator = \Validator::make($r->all(), [
            'token'=>'required',
        ...
        $data = Voucher::where('voucher_number',$r->voucher_number)->first();

        if(!$data) {
            return response()->json(['status'=>'failed','message'=>'Voucher tidak ditemukan'], 200);
        }

        if($data->status==1) {
            return response()->json(['status'=>'failed','message'=>'Voucher sudah digunakan'], 200);
        }

        return response()->json(['status'=>'success','amount'=>$data->amount], 200);
    }
}