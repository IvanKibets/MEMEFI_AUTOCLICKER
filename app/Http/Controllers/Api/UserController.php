...
class UserController extends Controller
{
    ...
    public function loginToko(Request $r)
    {
        if($r->username =="" or $r->password == "") return response(['status'=>401,'message'=>'Unauthorised : '. $r->email. ' : '. $r->password], 200);
        ...
        if(Auth::attempt([$field => $r->username, 'password' => $r->password])){
            ...
            $data['token'] =  \Auth::user()->createToken('STALAVISTA')->accessToken;
            $data['access_token'] =  \Auth::user()->createToken('STALAVISTA')->accessToken;
